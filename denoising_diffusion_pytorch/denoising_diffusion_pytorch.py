...
# 初始化WandB
# wandb.init(project='fashion_large_network', entity='why0725')
...
class Unet(nn.Module):
    ...
    # def forward(self, x, time=torch.full((1,), 10, device='cuda').long(), x_self_cond = None):  # 记得改回去
...
class GaussianDiffusion(nn.Module):
    ...
    def ddim_sample(self, shape, return_all_timesteps = False):
        ...
        # T_ = 900
        # time_pairs = [(i, i-1) for i in range(T_-1, 0, -1)]

        img = torch.randn(shape, device = device)
        '''
        img = Image.open('/mnt/store/lyx/github_projs/why/DDPM/mnist_200/image_1.png')
        transform = T.Compose(
            [
        ...
        time = 800
        t = torch.full((1,), time, device=device).long()
        noise = torch.rand_like(img)
        img = self.q_sample(x_start = img, t = t, noise = None).to(torch.float32)
        '''
        imgs = [img]
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        dl = DataLoader(self.ds, batch_size = train_batch_size, shuffle = True, pin_memory = True, num_workers = 3)
    ...
    def save(self, milestone):
        ...
        # save_path = str(self.results_folder/ f'model-{milestone}')# tbd
        # os.makedirs(save_path, exist_ok=True)

        torch.save(data, str(self.results_folder / f'model-{milestone}.pt'))
        # torch.save(data, os.path.join(save_path, f"model-{milestone}.pt"))
